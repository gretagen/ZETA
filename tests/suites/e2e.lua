...
suite:test("-DryRun resolves dependencies without installing", function()
  local e = fresh_env()
  local code, out = lib.run_zeta({ "-DryRun", "app" }, e)
  lib.assert_eq(code, 0, out)
  lib.assert_contains(out, "Would install hello-1.0")
  lib.assert_contains(out, "Would install app-1.0")
  lib.assert_true(not lib.exists(path.join(e.ZETA_ROOT, "usr/bin/app")))
  local db = query_db(e.ZETA_ROOT)
  lib.assert_true(not db.is_installed("hello"))
  lib.assert_true(not db.is_installed("app"))
end)
