...
suite:test("a dependency still required is protected like a package", function()
  local e = fresh_env()
  lib.run_zeta({ "-Provide", "app", "--pass" }, e)
...
suite:test("a dependency of several packages names all of them", function()
  local e = fresh_env()
  lib.run_zeta({ "-Provide", "app", "--pass" }, e)
  lib.run_zeta({ "-Provide", "app3", "--pass" }, e) -- both depend on hello
  local code, out = lib.run_zeta({ "-Remove", "hello", "--pass" }, e)
  lib.assert_eq(code, 1)
  lib.assert_contains(out, "app, app3")
  lib.assert_true(lib.exists(path.join(e.ZETA_ROOT, "usr/bin/hello")))
end)

suite:test("--force overrides the guard but still confirms", function()
  local e = fresh_env()
  lib.run_zeta({ "-Provide", "app", "--pass" }, e)
  -- --force bypasses the reverse-dependency guard, but the plan must
  -- still be confirmed: empty Enter cancels and nothing is removed.
  local code, out = lib.run_zeta({ "-Remove", "hello", "--force" }, e, "\n")
  lib.assert_eq(code, 0)
  lib.assert_contains(out, "will remove hello-1.0")
  lib.assert_contains(out, "breaks: app")
  lib.assert_contains(out, "aborted by user")
  lib.assert_true(lib.exists(path.join(e.ZETA_ROOT, "usr/bin/hello")))
  -- an explicit yes proceeds.
  local code2, out2 = lib.run_zeta({ "-Remove", "hello", "--force" }, e, "y\n")
  lib.assert_eq(code2, 0, out2)
  lib.assert_false(lib.exists(path.join(e.ZETA_ROOT, "usr/bin/hello")))
end)

suite:test("an orphaned dependency still confirms before removal", function()
  local e = fresh_env()
  lib.run_zeta({ "-Provide", "app", "--pass" }, e)
  lib.run_zeta({ "-Remove", "app", "--pass" }, e) -- hello left orphaned
  -- empty Enter cancels (default NO)
  local code, out = lib.run_zeta({ "-Remove", "hello" }, e, "\n")
  lib.assert_eq(code, 0)
  lib.assert_contains(out, "Remove 1 package")
  lib.assert_contains(out, "aborted by user")
  lib.assert_true(lib.exists(path.join(e.ZETA_ROOT, "usr/bin/hello")))
  -- explicit yes removes it
  local code2, out2 = lib.run_zeta({ "-Remove", "hello" }, e, "y\n")
  lib.assert_eq(code2, 0, out2)
  lib.assert_contains(out2, "removed hello")
  lib.assert_false(lib.exists(path.join(e.ZETA_ROOT, "usr/bin/hello")))
end)

suite:test("-Remove shows the file plan with --detail before confirming", function()
  local e = fresh_env()
  lib.run_zeta({ "-Provide", "hello", "--pass" }, e)
  local code, out = lib.run_zeta({ "-Remove", "hello", "--detail" }, e, "n\n")
  lib.assert_eq(code, 0)
  lib.assert_contains(out, "will remove hello-1.0 (1 file)")
  lib.assert_contains(out, "usr/bin/hello")
  lib.assert_contains(out, "Remove 1 package")
  lib.assert_contains(out, "aborted by user")
  lib.assert_true(lib.exists(path.join(e.ZETA_ROOT, "usr/bin/hello")))
end)
...
suite:test("--with-deps keeps dependencies shared with installed packages", function()
  ...
  local code, out = lib.run_zeta({ "-Remove", "app", "--pass", "--with-deps" }, e)
  lib.assert_eq(code, 0, out)
  lib.assert_contains(out, "keep hello (still required by app3)")
  lib.assert_true(lib.exists(path.join(e.ZETA_ROOT, "usr/bin/hello")))
  lib.assert_false(lib.exists(path.join(e.ZETA_ROOT, "usr/bin/app")))
...
suite:test("-Remove validates all names before removing anything", function()
  local e = fresh_env()
  lib.run_zeta({ "-Provide", "hello", "--pass" }, e)
