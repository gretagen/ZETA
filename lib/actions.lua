...
function actions.list()
  ...
  if #packages > 0 then
    ...
    if #dependencies > 0 then print("") end
  end
...
local function remove_one(name, via_cascade, flags, state)
  if state.removed[name] then
    if not via_cascade then log.info(("%s already removed, skipping"):format(name)) end
    return true, "skip"
  end
  local kind = db.kind(name)
  if not kind then return nil, ("%s is not installed"):format(name) end

  -- Dependents still being removed in this same invocation do not block.
  local rd = {}
  for _, d in ipairs(db.reverse_dependents(name)) do
    if not state.planned[d] then rd[#rd + 1] = d end
  end
  ...
  if #rd > 0 and not flags.force then
    ...
    if kind == "package" then
      return nil, ("cannot remove %s: still required by %s (use --force to override)"):format(
  ...
  if not proceed then
    ...
    elseif kind == "dependency" then
      ...
      proceed = confirm(
        ("%s is a dependency of %s, Proceed with removal?"):format(name, who), false, " [y/N]")
    else
  ...
  for _, d in ipairs(m and m.deps or {}) do db.remove_dependent(d, name) end
...
function actions.remove(names, flags)
  ...
  for _, n in ipairs(names) do state.planned[n] = true end
  for _, raw in ipairs(names) do
    local name = path.sanitize_name(raw)
    if not name then
      log.error("invalid package name: " .. tostring(raw))
